...
<cfset myFusebox.thisFuseaction = "delete_teacher">
<cftry>
<cfoutput><cfinclude template="../model/teachers/delete_teacher.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 18 and right(cfcatch.MissingFileName,18) is "delete_teacher.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse delete_teacher.cfm in circuit m_teachers which does not exist (from fuseaction m_teachers.delete_teacher).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
