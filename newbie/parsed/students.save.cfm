...
<cfset myFusebox.thisCircuit = "m_students">
<cfset myFusebox.thisFuseaction = "add_student">
<cftry>
<cfoutput><cfinclude template="../model/students/get_student_add.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 19 and right(cfcatch.MissingFileName,19) is "get_student_add.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse get_student_add.cfm in circuit m_students which does not exist (from fuseaction m_students.add_student).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
