...
<cfset myFusebox.thisCircuit = "m_courses">
<cfset myFusebox.thisFuseaction = "add_course">
<cftry>
<cfoutput><cfinclude template="../model/courses/get_course_add.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 18 and right(cfcatch.MissingFileName,18) is "get_course_add.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse get_course_add.cfm in circuit m_courses which does not exist (from fuseaction m_courses.add_course).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
