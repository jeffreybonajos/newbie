...
<cfset myFusebox.thisFuseaction = "get_teacher_by_id">
<cftry>
<cfoutput><cfinclude template="../model/teachers/get_teacher_id.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 18 and right(cfcatch.MissingFileName,18) is "get_teacher_id.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse get_teacher_id.cfm in circuit m_teachers which does not exist (from fuseaction m_teachers.get_teacher_by_id).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
...
<!--- do action="v_teachers.view_form" --->
<cfset myFusebox.thisFuseaction = "view_form">
<cfsavecontent variable="body_content">
<cftry>
<cfoutput><cfinclude template="../view/teachers/dsp_edit_form.cfm"></cfoutput>
...
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_edit_form.cfm in circuit v_teachers which does not exist (from fuseaction v_teachers.view_form).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
</cfsavecontent>
