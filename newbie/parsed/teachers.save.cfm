...
<cfset myFusebox.thisFuseaction = "add_teacher">
<cftry>
<cfoutput><cfinclude template="../model/teachers/get_teacher_add.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 19 and right(cfcatch.MissingFileName,19) is "get_teacher_add.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse get_teacher_add.cfm in circuit m_teachers which does not exist (from fuseaction m_teachers.add_teacher).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
