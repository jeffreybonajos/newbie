...
<cfset myFusebox.thisCircuit = "m_courses">
<cfset myFusebox.thisFuseaction = "delete_course">
<cftry>
<cfoutput><cfinclude template="../model/courses/delete_course.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 17 and right(cfcatch.MissingFileName,17) is "delete_course.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse delete_course.cfm in circuit m_courses which does not exist (from fuseaction m_courses.delete_course).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
