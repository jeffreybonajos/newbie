...
<cfset myFusebox.thisCircuit = "m_students">
<cfset myFusebox.thisFuseaction = "delete_student">
<cftry>
<cfoutput><cfinclude template="../model/students/delete_student.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 18 and right(cfcatch.MissingFileName,18) is "delete_student.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse delete_student.cfm in circuit m_students which does not exist (from fuseaction m_students.delete_student).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
